""" PyTorch DFF MAE (masked autoencoder) model."""
...
@dataclass
class DFFModelOutput(ModelOutput):
    """
    Class for DFFModel's outputs, with potential hidden states and attentions.
...
@dataclass
class DFFDecoderOutput(ModelOutput):
    """
    Class for DFFDecoder's outputs, with potential hidden states and attentions.
...
@dataclass
class DFFForPreTrainingOutput(ModelOutput):
    """
    Class for DFFForPreTraining's outputs, with potential hidden states and attentions.
...
@dataclass
class DFFClassifierOutput(ModelOutput):
    """
...
@dataclass
class DFFJointClassificationRegressionOutput(ModelOutput):
    """
    """
    loss: Optional[torch.FloatTensor] = None
...
class DFFEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.patch_embeddings = DFFPatchEmbeddings(config)
...
class DFFPatchEmbeddings(nn.Module):
    """
...
class DFFSelfAttention(nn.Module):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__()
...
class DFFSelfOutput(nn.Module):
    """
    The residual connection is defined in DFFLayer instead of here (as is the case with other models), due to the
...
class DFFAttention(nn.Module):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__()
        self.attention = DFFSelfAttention(config)
        self.output = DFFSelfOutput(config)
...
class DFFIntermediate(nn.Module):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__()
...
class DFFLayer(nn.Module):
    ...
    def __init__(self, config: DFFConfig) -> None:
        ...
        self.seq_len_dim = 1
        self.attention = DFFAttention(config)
        self.intermediate = DFFIntermediate(config)
        self.output = DFFOutput(config)
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            self.layernorm_before(hidden_states),  # in DFF, layernorm is applied before self-attention
            head_mask,
        ...
        # in DFF, layernorm is also applied after self-attention
...
class DFFEncoder(nn.Module):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([DFFLayer(config) for _ in range(config.num_hidden_layers)])
...
class DFFPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = DFFConfig
    base_model_prefix = "dff"
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, DFFEncoder):
            module.gradient_checkpointing = value


class DFFModel(DFFPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = DFFEmbeddings(config)
        self.encoder = DFFEncoder(config)
    ...
    def forward(
        ...
        r"""
            ...
            ```python
            >>> from transformers import AutoFeatureExtractor, DFFModel
            ...
            >>> feature_extractor = AutoFeatureExtractor.from_pretrained("facebook/dff-mae-base")
            >>> model = DFFModel.from_pretrained("facebook/dff-mae-base")
...
class DFFDecoder(nn.Module):
    def __init__(self, config, num_filtration_para):
        ...
        self.decoder_layers = nn.ModuleList(
            [DFFLayer(decoder_config) for _ in range(config.decoder_num_hidden_layers)]
        )
...
class DFFForPreTraining(DFFPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.dff = DFFModel(config)
        self.decoder = DFFDecoder(config, num_filtration_para=self.dff.embeddings.num_filtration_para)

        # Initialize weights and apply final processing
        self.post_init()

    def get_input_embeddings(self):
        return self.dff.embeddings.patch_embeddings
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple, DFFForPreTrainingOutput]:
        r"""
            Returns:
            ...
            ```python
            >>> from transformers import AutoFeatureExtractor, DFFForPreTraining
            >>> from PIL import Image
            >>> import requests
        ...
        return DFFForPreTrainingOutput(
            loss=loss,
            logits=logits,
            mask=mask,
            ids_restore=ids_restore,
...
class DFFPooler(DFFPreTrainedModel):
    def __init__(self, config: DFFConfig):
        super().__init__(config)
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class DFFForImageClassification(DFFPreTrainedModel):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__(config)

        self.num_labels = config.num_labels
        self.dff = DFFModel(config)
        self.pooler = DFFPooler(config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, DFFClassifierOutput]:
        r"""
...
class DFFForJointClassificationRegression(DFFPreTrainedModel):
    def __init__(self, config: DFFConfig) -> None:
        super().__init__(config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, DFFJointClassificationRegressionOutput]:
        r"""
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the image classification/regression loss. Indices should be in `[0, ...,
            config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss), If
        ...
        return DFFJointClassificationRegressionOutput(
            loss=loss,
...
class DFFFForClassificationReconstruction(DFFPreTrainedModel):
